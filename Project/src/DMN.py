...
class DMN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, dropout_p=0.1):
        ...
        self.attn_weights = nn.Sequential(nn.Linear(4 * hidden_size, hidden_size), nn.Tanh(), nn.Linear(hidden_size, 1),
                                          nn.Softmax())
    ...
    def forward(self, facts, facts_masks, question, question_masks, word_to_index, num_decode, episodes=3, is_training=True):
        # input module
        concated = []
        ...
        if is_training == True:
            ...
            encoded_question = torch.cat(real_question).view(question.size(0), -1)  # B,D
